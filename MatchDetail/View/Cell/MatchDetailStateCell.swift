...
@MainActor
final class MatchDetailStateCell: DetailBaseCollectionViewCell {
    ...
    func configure(state: MatchDetailViewState) {
        switch state {
        case .idle, .loading:
            isUserInteractionEnabled = false
            hintView.isHidden = true
        ...
        case .failed(let failure):
            isUserInteractionEnabled = failure == .loadFailed
            activityIndicator.stopAnimating()
