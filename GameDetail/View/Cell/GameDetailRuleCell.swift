...
final class GameDetailRuleCell: DetailBaseCollectionViewCell {
    ...
    private var rules: [GameDetailRule] = []
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(GameDetailRuleItemCell.self)
    ...
    func configure(rules: [GameDetailRule]) {
        self.rules = rules
        collectionView.reloadData()
    }

    static func preferredHeight(
        for rules: [GameDetailRule],
        width: CGFloat
    ) -> CGFloat {
        rules.reduce(0) { height, rule in
            height + GameDetailRuleItemCell.preferredHeight(
                for: rule.text,
                width: width
            )
        }
...
extension GameDetailRuleCell: UICollectionViewDataSource {
...
extension GameDetailRuleCell: UICollectionViewDelegateFlowLayout {
...
@MainActor
private final class GameDetailRuleItemCell: DetailBaseCollectionViewCell {
